# Summer_Assignment(CDC)
### Student Details
- **Name:** Aditya
- **Roll No:** 2401920100035
- **Branch:** Computer Science and Engineering (CSE)
- **College:** G.L. Bajaj Institute of Technology and Management

---

## About
This repository contains my solutions for the 4-Week DSA & OOPS Summer Assignment.

I will be regularly updating this repository with solutions, time complexities, space complexities, and approaches used to solve the problems.

---

## Progress

### Progress Overview

- 🚧 In Progress: Week 4
- 🎯 Current Focus: Tree

---

### Week 1 — Arrays + Basic Strings

#### Day 1: Array Basics
- [x] [Two Sum](Week_1/Day_1/two_sum.cpp)
- [x] [Remove Duplicates from Sorted Array](Week_1/Day_1/remove_duplicates_from_sorted_array.cpp)
- [x] [Best Time to Buy and Sell Stock](Week_1/Day_1/best_time_toBuyandSellStock.cpp)

#### Day 2: Prefix Sum / Sliding Window
- [x] [Maximum Subarray](Week_1/Day_2/maximum_subarray.cpp)
- [x] [Contains Duplicate](Week_1/Day_2/contains_duplicate.cpp)
- [x] [Maximum Average Subarray I](Week_1/Day_2/maximumAverage_subarray_I.cpp)

#### Day 3: Two Pointer
- [x] [Move Zeroes](Week_1/Day_3/move_zeroes.cpp)
- [X] [Squares of a Sorted Array](Week_1/Day_3/squares_of_a_sorted_array.cpp)
- [x] [Container With Most Water](Week_1/Day_3/container_with_most_water.cpp)

#### Day 4: Matrix Problems
- [x] [Matrix Diagonal Sum](Week_1/Day_4/matrix_diagonal_sum.cpp)
- [x] [Reshape the Matrix](Week_1/Day_4/reshape_the_matrix.cpp)
- [x] [Spiral Matrix](Week_1/Day_4/spiral_matrix.cpp)

#### Day 5: String Basics
- [x] [Valid Palindrome](Week_1/Day_5/valid_palindrome.cpp)
- [x] [Reverse String](Week_1/Day_5/reverse_string.cpp)
- [x] [Longest Common Prefix](Week_1/Day_5/longest_common_prefix.cpp)

### Week 2 — Strings (Advanced)

#### Day 1: Hashing in Strings
- [x] [Valid Anagram](Week_2/Day_1/valid_anagram.cpp)
- [x] [First Unique Character in a String](Week_2/Day_1/first_unique_character_in_a_string.cpp)
- [x] [Ransom Note](Week_2/Day_1/ransom_note.cpp)

#### Day 2: Sliding Window on Strings
- [x] [Longest Substring Without Repeating Characters](Week_2/Day_2/longest_substring_without_repeating_characters.cpp)
- [x] [Permutation in String](Week_2/Day_2/permutation_in_string.cpp)
- [x] [Find All Anagrams in a String](Week_2/Day_2/find_all_anagrams_in_a_string.cpp)

#### Day 3: Pattern Matching
- [x] [Implement strStr()](Week_2/Day_3/implement_strstr.cpp)
- [x] [Is Subsequence](Week_2/Day_3/is_subsequence.cpp)
- [x] [Repeated Substring Pattern](Week_2/Day_3/repeated_substring_pattern.cpp)

#### Day 4: Recursion + Strings
- [x] [Reverse Words in a String III](Week_2/Day_4/reverse_words_in_a_string_iii.cpp)
- [x] [Decode String](Week_2/Day_4/decode_string.cpp)
- [x] [Generate Parentheses](Week_2/Day_4/generate_parentheses.cpp)

#### Day 5: Mixed String Problems
- [x] [Group Anagrams](Week_2/Day_5/group_anagrams.cpp)
- [x] [String Compression](Week_2/Day_5/string_compression.cpp)
- [x] [Longest Palindromic Substring](Week_2/Day_5/longest_palindromic_substring.cpp)

### Week 3 — Linked List + Stack + Queue

#### Day 1: Linked List
- [x] [Linked List Cycle](Week_3/Day_1/linked_list_cycle.cpp)
- [x] [Reverse Linked List](Week_3/Day_1/reverse_linked_list.cpp)
- [x] [Middle of the Linked List](Week_3/Day_1/middle_of_the_linked_list.cpp)

#### Day 2: Linked List
- [x] [Merge Two Sorted Lists](Week_3/Day_2/merge_two_sorted_lists.cpp)
- [x] [Remove Nth Node From End of List](Week_3/Day_2/remove_nth_node_from_end_of_list.cpp)
- [x] [Palindrome Linked List](Week_3/Day_2/palindrome_linked_list.cpp)

#### Day 3: Stack
- [x] [Valid Parentheses](Week_3/Day_3/valid_parentheses.cpp)
- [x] [Min Stack](Week_3/Day_3/min_stack.cpp)
- [x] [Next Greater Element I](Week_3/Day_3/next_greater_element_i.cpp)

#### Day 4: Stack
- [x] [Daily Temperatures](Week_3/Day_4/daily_temperatures.cpp)
- [x] [Evaluate Reverse Polish Notation](Week_3/Day_4/evaluate_reverse_polish_notation.cpp)
- [x] [Largest Rectangle in Histogram](Week_3/Day_4/largest_rectangle_in_histogram.cpp)

#### Day 5: Queue / Deque
- [x] [Implement Queue using Stacks](Week_3/Day_5/implement_queue_using_stacks.cpp)
- [x] [Number of Recent Calls](Week_3/Day_5/number_of_recent_calls.cpp)
- [x] [Sliding Window Maximum](Week_3/Day_5/sliding_window_maximum.cpp)

### Week 4 — Trees

#### Day 1: Binary Tree Basics
- [x] [Maximum Depth of Binary Tree](Week_4/Day_1/maximum_depth_of_binary_tree.cpp)
- [x] [Invert Binary Tree](Week_4/Day_1/invert_binary_tree.cpp)
- [x] [Same Tree](Week_4/Day_1/same_tree.cpp)

#### Day 2: Traversals
- [x] [Binary Tree Inorder Traversal](Week_4/Day_2/binary_tree_inorder_traversal.cpp)
- [x] [Binary Tree Level Order Traversal](Week_4/Day_2/binary_tree_level_order_traversal.cpp)
- [x] [Binary Tree Zigzag Level Order Traversal](Week_4/Day_2/binary_tree_zigzag_level_order_traversal.cpp)

#### Day 3: Binary Search Tree
- [x] [Search in a Binary Search Tree](Week_4/Day_3/search_in_a_binary_search_tree.cpp)
- [x] [Validate Binary Search Tree](Week_4/Day_3/validate_binary_search_tree.cpp)
- [x] [Lowest Common Ancestor of a BST](Week_4/Day_3/lowest_common_ancestor_of_a_bst.cpp)

#### Day 4: Path Based Problems
- [x] [Path Sum](Week_4/Day_4/path_sum.cpp)
- [x] [Diameter of Binary Tree](Week_4/Day_4/diameter_of_binary_tree.cpp)
- [x] [Binary Tree Maximum Path Sum](Week_4/Day_4/binary_tree_maximum_path_sum.cpp)

#### Day 5: Advanced Trees
- [x] [Symmetric Tree](Week_4/Day_5/symmetric_tree.cpp)
- [x] [Construct Binary Tree from Preorder and Inorder Traversal](Week_4/Day_5/construct_binary_tree_from_preorder_and_inorder_traversal.cpp)
- [x] [Serialize and Deserialize Binary Tree](Week_4/Day_5/serialize_and_deserialize_binary_tree.cpp)

## Repository Structure

```text
Week_1/
├── Day_1/
├── Day_2/
├── Day_3/
├── Day_4/
├── Day_5/
└── OOPS/

Week_2/
├── Day_1/
├── Day_2/
├── Day_3/
├── Day_4/
├── Day_5/
└── OOPS/

Week_3/
├── Day_1/
├── Day_2/
├── Day_3/
├── Day_4/
├── Day_5/
└── OOPS/
```

---

## Languages Used
- C++
- STL

---

## Goal
- Improve problem-solving skills
- Strengthen DSA concepts
- Build coding consistency
- Prepare for coding interviews and contests

---

Daily updates will be pushed throughout the assignment period.